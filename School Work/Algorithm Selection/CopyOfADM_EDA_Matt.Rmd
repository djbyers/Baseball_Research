...
games.trainx <- games.train3 %>%
  subset(select = -Home_KBB)
games.testx <- games.test3 %>%
  subset(select = -Home_KBB)
  

numTree <- 50
numPred <- ncol(games.trainx)-1

mod.bag <- randomForest(Home_Win ~ .,
                        data=games.trainx,
                        ntree=numTree,
                        mtry=numPred)
...
```{r}
view(bagtest.df)
bagtest.df <- games.testx %>%
  add_predictions(mod.bag,
                  var="win.pred",
                  type = "class") %>%
  mutate(win.pred = ifelse(win.pred > 0.5, 1, 0))
